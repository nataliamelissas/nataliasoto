  ...
  return (
    <article className="group relative flex flex-col h-full min-w-0 rounded-3xl bg-white border border-ivory-300/70 p-7 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:border-butter-400 hover:shadow-lift">
      <div className="flex items-start justify-between mb-4 gap-3">
        ...
        {project.status && (
          <span
            className={`shrink-0 text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-full border ${STATUS_STYLE[project.status]}`}
          >
      ...
      <p className="text-ink-400 text-sm leading-relaxed mb-5 flex-grow">
        {project.description}
      </p>
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        {project.tags.map((tag) => (
          <span
            key={tag}
            className="text-[11px] uppercase tracking-wider text-ink-600 bg-ivory-200 border border-ivory-300 px-2 py-0.5 rounded-full"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex items-center gap-5 pt-4 border-t border-ivory-300/70">
        <a
          href={project.repo}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-1.5 text-sm font-medium text-ink-700 hover:text-butter-500 transition-colors"
        >
